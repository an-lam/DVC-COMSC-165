...
int main( )
{
  ...
  cout << "CAPACITY = " << CAPACITY << endl; // is it changed by func?
 
  //CAPACITY = 200;  // compiling error becase of 'const'
...
int gatherInput(int theArray[], int capacity)
{
  ...
  capacity = 500; // for fun !
  
  return result;
}

// a generic statistics function -- scores, temperatures, etc...
void getStatistics(int a[], int n, double &average, int &max, int &min)
{
  int sum = a[0]; // add up all scores here, and divide by n
  max = a[0]; // as far as we know, the zeroth score is the max
  min = a[0]; // as far as we know, the zeroth score is the min, too
  ...
  average = 1.0 * sum / n; // "1.0" so result will be floating point
}